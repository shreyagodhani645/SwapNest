...
async function check() {
    try {
        ...
        const cats = await db.execute("SELECT * FROM CATEGORIES", [], { outFormat: 4002 });
        console.log(cats.rows);
        
        console.log("\n--- Listing Counts by Category Index ---");
        const counts = await db.execute("SELECT CATEGORY_ID, COUNT(*) as COUNT FROM LISTINGS GROUP BY CATEGORY_ID", [], { outFormat: 4002 });
        ...
        if (joined.rows.length === 0) {
            ...
            const orphanCount = await db.execute("SELECT COUNT(*) as COUNT FROM LISTINGS WHERE CATEGORY_ID NOT IN (SELECT ID FROM CATEGORIES)", [], { outFormat: 4002 });
