...
const MyListings = () => {
  ...
  const handleStatusUpdate = async (offerId, newStatus) => {
    try {
      ...
      fetchData(); // Refresh list to reflect the new status
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-gray-100">
            <p className="text-xl text-gray-500 font-medium">You have no listings</p>
          </div>
        ) : (
          <div className="space-y-8">
            {groupedListings.map((group, idx) => (
              <div key={idx} className="bg-white rounded-[2rem] shadow-premium p-8 border border-gray-100">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 border-b pb-4 gap-4">
                  <div>
                    <h2 className="text-2xl font-black text-gray-800">{group.TITLE}</h2>
                    <p className="text-gray-500 font-bold mt-1">9{group.PRICE.toLocaleString()} &bull; {group.CONDITION}</p>
                  </div>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => window.location.href = `/listings/${String(group.ID)}`}
                      className="text-sm font-bold text-primary hover:text-primary-dark border border-primary/20 px-4 py-2 rounded-xl"
                    >
                      View Details
                    </button>
                    <button 
                    ...
                    <button
                      ...
                      className="text-sm font-bold text-red-600 hover:text-white hover:bg-red-600 border border-red-200 px-4 py-2 rounded-xl transition-all"
                    >
                      Delete
                    </button>
                  </div>
                ...
                <div className="space-y-4">
                  {group.offers.length === 0 ? (
                    <p className="text-gray-400 italic font-medium px-4">No offers received yet.</p>
                  ) : group.offers.map(offer => (
                    <div key={offer.ID} className="bg-gray-50 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border border-gray-100 shadow-sm transition-all hover:shadow-md">
                      <div>
                        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-1">Buyer: {offer.BUYER_NAME}</p>
